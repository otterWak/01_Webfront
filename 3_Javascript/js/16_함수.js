...
function returnTest(num1, num2){
  return num1*num2; // 함수 종료 + return옆에 작성된 값을 가지고
                    // 호출한 쪽으로 돌아감
}

function sumFn(sumTest){
  
  let sum = 0;
  for(let i = 0; i < sumTest.length; i++){
    sum += sumTest[i];
  }
...
document.querySelector("#btn3a").addEventListener("click", function(){
  ...
  numbers.push(50);
  numbers.push(4);
  numbers.push( returnTest(2, 5) ); //함수를 호출해서 되돌려받은 값(retun rkqt;)을 push 하기

  console.log(numbers); // [30, 50, 4, 10]
...
function printObject(otherFn){
  // otherFn = return{"name" : name, "age" : age}
