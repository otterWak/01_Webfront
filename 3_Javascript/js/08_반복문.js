...
function check16(){
  ...
  while(val !== null){ // null(prompt창에서 취소 버튼 누를 때 까지)
    // != 비교 연산자 : 값을 비교
    // 동일 비교 연산자
...
function check17(){
  // 메뉴 가격 변수
  const gimbap = 3000; // 김밥 가격
  const ramen = 3500; // 라면 가격
  const donkkaseu = 5000; // 돈까스 가격
