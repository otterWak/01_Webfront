...
// 작성법
// 변수명 = 값;
number1 = 10; //오른쪽에 있는 값을 왼쪽에 대입하는 것
// console에 number1값 출력하기
console.log(number1);
...
// 형변환 없이 출력되어 문자열 따로, 다른 데이터 타입 따로 출력됨
...
// const PI;
// const test;
// 'const' declarations must be initialized(const 선언은 초기화 되어야 합니다)
...
console.log(foo1); // 123, 전역변수이기 때문에 (블로고가 관계없음) 접근 가능
console.log(foo2); // 456, 전역변수이기 때문에 (블로고가 관계없음) 접근 가능
...
// 제한이 없으면 코드 짜기가 어려워서 요즘은 잘 사용하지 않음
...
example(); //함수의 호출
//호출을 해야 함수가 수행되면서 함수내에 작성된 코드가 수행됨
...
// JS의 배열은 [] 작성, ','로 구분해서 열 작성
const numbers = [10, 20, 30]; //0 index, 1 index, 2 index
...
// undefined - numbers[3] 에는 아무값도 할당외지 않아 undefined가 된다.
...
const sumFn = function(a, b) { return a + b; }; //반환한다 : 나를 호출한쪽으로
