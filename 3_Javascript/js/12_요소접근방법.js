...
function classTest(){
  ...
  divs[0].style.backgroundColor = "#cdb4db";
  divs[1].style.backgroundColor = "#ffafcc";
  divs[2].style.backgroundColor = "#a2d2ff";
}
...
function cssTest(){
  ...
  // [요소 1개]
  // 1) document.querySellector("CSS 선택자");
  //    -> 선택자가 선택한 요소 중 첫번째 요소를 반환

  // [요소 n개 -> 배열]
  // 2) document.querySelectorAll("CSS 선택자");
  //    -> 선택한 요소 모두를 배열(유사 배열) 형태로 전환
  ...
  div1.innerText = `CSS 선택자를 선택해서 값 변경됨`;
...
// 올라온 키카 "Enter"키 이면 readValuer() 함수를 호출

// keyup : 눌러지던 키가 떼어졌을 때(올라왔을 때). 1회만 인식
// keydown : 키가 눌러졌을 때 ( + 꾹 누르고 있으면 계속 인식)
